...
// Constructor
//----------------
eicmcio::BeagleReader::BeagleReader(const std::string& source_name)
{
    ...
    text_reader = std::unique_ptr<eicmcio::TextEventFileReader>(new TextEventFileReader(
            ...
            [](uint64_t line_num, const std::string& line) {
                ...
                auto trimmed_line = eicmcio::TrimCopy(line);

                // skip those ===== in the beginning and end of events
                if(eicmcio::StartsWith(trimmed_line, "="))  {
    ...
    fmt::print("EventSourceControl: created \n");
}
...
bool BeagleReader::GetEvent(BeagleEventData& event_data)
{
    ...
    // Do we have to stop?
    if (decision == EventControlDecisions::Stop) return false;
    ...
    for(auto particle_tokens : event_data.text_event->particle_values)
    {
        ...
        event_data.particles.push_back(particle);
    }
...
template<>
void JFactoryT<eicmcio::BeagleParticle>::Process(const std::shared_ptr<const JEvent>& event) {

    auto text_event_record = event->GetSingle<eicmcio::BeagleEventData>();
...
void JFactoryT<minimodel::McGeneratedParticle>::Process(const std::shared_ptr<const JEvent>& event) {
    ...
    auto beagle_particles = event->Get<eicmcio::BeagleParticle>();
